...
Public Class MainDialog
    ...
    Private Sub MainDialog_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ...
        If System.IO.File.Exists(Application.StartupPath + "\SignAPI.dll") Then
        ...
        Else
            MsgBox("SignAPI.dll not found!", MsgBoxStyle.Critical, "Error!")
            Me.Close()
        End If
    ...
    Public Sub SignFunction()
        ...
        If System.IO.File.Exists(Application.StartupPath + "\SignAPI.dll") Then
            If TimestampCheck.Checked = True Then
                ...
                SignProcess.StartInfo.Arguments = "/k echo off" & CommandSeperator & "echo SignAPI CommandLine v1.0" & CommandSeperator & "echo." & CommandSeperator & "echo Author: Debdut Biswas" & CommandSeperator & "echo Copyright(C) Dev Software Development " & Date.Today.Year & CommandSeperator & "echo." & CommandSeperator & "SignAPI.dll sign /f " & PFX & " /p " & Password & " /d " & Discription & " /du " & DiscriptionURL & " /i " & IssuarName & " /n " & SubjectName & " /fd " & FDA & " /t " & TSA & " " & Win32PE & CommandSeperator & "echo Signing and TimeStamping Time: " & Date.Now
            ...
            Else
                Dim SignProcess = New Process
                SignProcess.StartInfo.FileName = Environment.GetFolderPath(Environment.SpecialFolder.System) & "\cmd.exe"
                SignProcess.StartInfo.Arguments = "/k echo off" & CommandSeperator & "echo SignAPI CommandLine v1.0" & CommandSeperator & "echo." & CommandSeperator & "echo Author: Debdut Biswas" & CommandSeperator & "echo Copyright(C) Dev Software Development " & Date.Today.Year & CommandSeperator & "echo." & CommandSeperator & "SignAPI.dll sign /f " & PFX & " /p " & Password & " /d " & Discription & " /du " & DiscriptionURL & " /i " & IssuarName & " /n " & SubjectName & " /fd " & FDA & " " & Win32PE & CommandSeperator & "echo Signing Time: " & Date.Now
                SignProcess.StartInfo.UseShellExecute = False
                SignProcess.StartInfo.CreateNoWindow = True
                SignProcess.StartInfo.RedirectStandardOutput = True
                SignProcess.StartInfo.RedirectStandardError = True
